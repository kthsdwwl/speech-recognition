...
void captureFeatAndWriteToFile()
{
	...
	outputToFile(powerSpec.submat(0, 0, powerSpec.n_rows / 2, powerSpec.n_cols - 1), "powerSpec.txt");
	outputToFile(audSpec, "audSpec.txt");
	outputToFile(ceps, "ceps.txt");
	outputToFile(featureVecs, "featureVecs.txt");
	printf("Done\n");
}

void wav2feat(char* filename, double result[][feat_num], int &len)
{
	...
	len = featureVecs.n_cols;
	for (int i = 0; i < feat_num; ++i)
		for (int j = 0; j < len; ++j)
			result[j+1][i] = featureVecs.at(i, j);
}
